package ryu.useless.project03.config;
...
public class DataSetupConfig {
    private static final Logger logger = LoggerFactory.getLogger(DataSetupConfig.class);

    private static HikariConfig config = new HikariConfig();
    private static HikariDataSource ds;

    private DataSetupConfig() {
    }
    ...
    public static synchronized void initializeDataSource(String appProperties) {
        if (appProperties == null) {
            try (InputStream resourceStream = DataSetupConfig.class.getClassLoader().getResourceAsStream("application.properties")) {
