-- ---------------------------------------------------------------------
-- "computer_store_dtbs" Project 02 Zacek Jan Filip (Anakin Spacerunner)
-- ---------------------------------------------------------------------
-- SCHEMA bds :)
...
DROP ROLE IF EXISTS "pies";
/* just for making sure, that there is no active role with the same name */
CREATE ROLE "pies" WITH LOGIN NOSUPERUSER ENCRYPTED PASSWORD 'Kotek';
/* next commands should work without "peis" thing, if not, simply add "" there */
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA bds TO pies;
GRANT USAGE ON schema bds TO pies;
grant all privileges on all sequences in schema bds to pies;
...
CREATE TABLE IF NOT EXISTS "dummytable" (
    "id_dummytable" SERIAL NOT NULL,
  "capybara01" TEXT NOT NULL,
  "capybara02" TEXT NOT NULL,
  PRIMARY KEY ("id_dummytable")
);
grant all on table dummytable to pies;
ALTER TABLE bds.dummytable OWNER TO pies;

/*Inserts for swl dummy table are in the DML*/
