...
class Encoder:
    def __init__(self):
        while True:
            try:
                self.__file_in__ = input("Enter image name (with extension) : ")  # change to input or connect to gui
                self.__image__ = Image.open(self.__file_in__)
                break
        ...
        while True:
            try:
                self.__msg__ = input("Enter data to be encoded (maximum lenght is 255 characters, no special characters allowed) : ")
    ...
    def encode(self):
        ...
        while True:
            try:
                self.__file_out__ = input("Enter the name of new image (with extension) : ")
    ...
    def __file_hash(self):
        ...
        with open(self.__file_in__, "rb") as f:
            while True:
